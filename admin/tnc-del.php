...
if(count($arr)>0){
...

	{
	    ...
	    $sql="delete from $tbl_tnc where id in ($str_rest_refs)"; 
	...
	{	
		...
		$sql="update $tbl_tnc set status=1 where id in ($str_rest_refs)";
	...
	{		
		 ...
		 $sql="update $tbl_tnc set status=0 where id in ($str_rest_refs)";
