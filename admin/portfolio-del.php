...
if(count($arr)>0){
	...
	if($Submit=='Delete')
	{  
	    $sql="delete from $tbl_portfolio where id in ($str_rest_refs)"; 
		$obj->query($sql);
	...
	{	
		$sql="update $tbl_portfolio set status=1 where id in ($str_rest_refs)";
		$obj->query($sql);
	...
	{		
		$sql="update $tbl_portfolio set status=0 where id in ($str_rest_refs)";
		$obj->query($sql);
